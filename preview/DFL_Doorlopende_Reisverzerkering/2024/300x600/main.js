...
function getUSPAnimation(){
    ...
    uspTL.from("#uspScreen", 0.5, {height:166, ease:Sine.easeInOut});
...
function getQuestionAnimation(){
    ...
    questionTL.from("#questionScreen", 0.5, {height:166, ease:Sine.easeInOut});
...
function calcPrices(){
    ...
    if(wereld && maand && klant){
        ...
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>2,40</span> p.p.</span> per maand"
    }
    else if(wereld && !maand && klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=we&bt=jr&kl=ja';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>28,80</span> p.p.</span> per jaar"
    }
    
    if(!wereld && maand && klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=eu&bt=mnd&kl=ja';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>1,60</span> p.p.</span> per maand"
    }
    else if(!wereld && !maand && klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=eu&bt=jr&kl=ja';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>19.90</span> p.p.</span> per jaar"
    }

    if(wereld && maand && !klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=we&bt=mnd&kl=nee';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>2,67</span> p.p.</span> per maand"
    }
    else if(wereld && !maand && !klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=we&bt=jr&kl=nee';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>32,00</span> p.p.</span> per jaar"
    }
    
    if(!wereld && maand && !klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=eu&bt=mnd&kl=nee';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>1,78</span> p.p.</span> per maand"
    }
    else if(!wereld && !maand && !klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=eu&bt=yr&kl=nee';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>21,33</span> p.p.</span> per jaar"
    }
}
