...
function getAnimation(){
    ...
    mainTL.to("#redLine", 0.3, {width:0, ease:Power3.easeInOut, onStart:function(){
        gsap.set("#redLine", {left:"auto", right:93,});
    }}, "start+=5.2")
    mainTL.to("#panel", 0.5, {height:323, ease:Sine.easeInOut}, "start+=5.3")
    mainTL.from(splitT2.words, 0.3, {opacity: 0, x:-20, ease: Sine.easeOut, stagger: 0.1}, "start+=5.7");
    mainTL.to("#redLine", 0.3, {width:45, ease:Power3.easeInOut, onStart:function(){
        gsap.set("#redLine", {left:20, bottom:217, right:"auto",});
    }}, "start+=6.5")
...
function createSnow(){
    ...
    for (var i = 0; i < 700; i++) {
        ...
        newDiv.style.left = gsap.utils.random(0, 160)+"px";
        ...
        gsap.to(newDiv, 6, {x:gsap.utils.random(-250, 250), ease:Power0.easeNone})
