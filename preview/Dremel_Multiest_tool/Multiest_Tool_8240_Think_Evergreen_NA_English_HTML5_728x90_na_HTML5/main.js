...
function getAnimation(){
    ...
    masterTL.from("#logo", 0.5, {width:200, bottom:"50%", y:"50%", left:"50%", x:"-50%", ease:Sine.easeOut}, "start+=0.5")
    ...
    masterTL.from("#product", 0.5, {y:100, ease:Back.easeOut}, "showProduct5")
    for(var i=1; i<6; i++){
        masterTL.from("#tool"+i+" .toolText", 0.5, {opacity:0, ease:Sine.easeOut}, "showProduct5+="+(0.3*(i+1)))
        masterTL.from("#tool"+i+" .toolImage", 0.5, {opacity:0, stagger:0.1, ease:Sine.easeOut}, "showProduct5+="+(0.3*(i+1)))
    }
    // masterTL.add("caroussel")
    // for(var i=1; i<6; i++){
    //     masterTL.to("#toolsWrapper", 0.5, {x:"-=60", ease:Sine.easeInOut}, "caroussel+="+(i-1)*1)
