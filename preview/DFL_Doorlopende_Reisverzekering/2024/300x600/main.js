...
function getUSPAnimation(){
    ...
    uspTL.from("#uspScreen", 0.5, { height: 166, y: -2, ease: Sine.easeInOut });
    uspTL.to("#bg", 0.5, {scale:1.2, ease:Sine.easeOut}, "-=0.5")
    uspTL.to("#stickerWrapper", 0.5, {scale:1, opacity:1, y: -247, x: 4, rotation:0, ease:Sine.easeOut})
...
function getQuestionAnimation(){
    ...
    questionTL.from("#questionScreen", 0.5, {height:167, y:-2, ease:Sine.easeInOut});
    questionTL.to("#stickerWrapper", 0.5, {scale:1, opacity:1, y: -317, x: 4, rotation:0, ease:Sine.easeOut})
...
function calcPrices(){
    ...
    if(wereld && maand && klant){
        ...
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>2,27</span> p.p.</span> per maand"
    }
    else if(wereld && !maand && klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=we&bt=jr&kl=ja';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>27,21</span> p.p.</span> per jaar"
    }
    
    if(!wereld && maand && klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=eu&bt=mnd&kl=ja';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>1,52</span> p.p.</span> per maand"
    }
    else if(!wereld && !maand && klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=eu&bt=jr&kl=ja';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>18,13</span> p.p.</span> per jaar"
    }

    if(wereld && maand && !klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=we&bt=mnd&kl=nee';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>2,52</span> p.p.</span> per maand"
    }
    else if(wereld && !maand && !klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=we&bt=jr&kl=nee';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>30,23</span> p.p.</span> per jaar"
    }
    
    if(!wereld && maand && !klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=eu&bt=mnd&kl=nee';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>1,68</span> p.p.</span> per maand"
    }
    else if(!wereld && !maand && !klant){
        window.newExitUrl = 'https://reisverzekeringen.defriesland.nl/doorlopend?rv=nee&dg=eu&bt=yr&kl=nee';
        document.querySelector("#cta2 .ctaText").innerHTML = "<span>€ <span id='price'>20,14</span> p.p.</span> per jaar"
    }
}
